...
export default class ColorService {
    ...
    public static CssToRGBA(cssStr: string): string {
        let colorData: Uint8ClampedArray = this._getRGBA(cssStr);
    ...
    public static GetBrightness(colorStr: string): number {
        let colorData: Uint8ClampedArray = this._getRGBA(colorStr);
    ...
    public static GetBrightnessIndex(colorStr: string, numSteps: number): number {
        ...
        const index = (val => {
            let retArray = []; for(let x=0; x<=255; x+=(255/val)) { retArray.push(x); } return retArray;
